...
public class Util {
	...
	private static DateFormat mJpegDateFormat = new SimpleDateFormat(
			"yyyyMMdd_HHmmssSSS");

	/**
	 * Returns the orientation of the display In our case, since we're locked in
	 * Landscape, it should always be 90
	 * 
	 * @param activity
	...
	/**
	 * Rounds the orientation so that the UI doesn't rotate if the user holds
	 * the device towards the floor or the sky
	 * 
	 * @param orientation
	 *            New orientation
	...
	public static Point getScreenSize(Activity activity) {
		if (activity != null) {
			WindowManager service = (WindowManager) activity
	...
	public static Bitmap decodeYUV420SP(Context context, byte[] yuv420sp,
		...
		//
		// Log.v(TAG, "allocationIn " + allocationIn.getBytesSize());
		// Log.v(TAG, "allocationOut " + allocationOut.getBytesSize());
	...
	/**
	 * Returns the physical path (on emmc/sd) of the provided URI from
	 * MediaGallery
	 * 
	 * @param context
	 * @param contentURI
	 ...
	 * @return
	 */
	public static Point findBestPanoPreviewSize(List<Size> supportedSizes,
			boolean need4To3, boolean needSmaller, int defaultPixels) {
		Point output = null;
	...
	/**
	 * Returns the best PicSphere picture size. The reference size is 2048x1536
	 * from Nexus 4.
	 * 
	 * @param supportedSizes
	 *            Supported picture size
	 * @param needSmaller
	 *            If a larger image size is accepted
	 * @return A Point where X and Y corresponds to Width and Height
	 */
	public static Point findBestPicSpherePictureSize(List<Size> supportedSizes,
			boolean needSmaller) {
		Point output = null;
	...
	/**
	 * Older devices need to stop preview before taking a shot (example: galaxy
	 * S, galaxy S2, etc)
	 * 
	...
	public static boolean deviceNeedsStopPreviewToShoot() {
		...
		Log.e(TAG, "Device " + Build.BOARD
				+ (needs ? " needs " : " doesn't need ") + "to stop preview");
		return needs;
	}
}
