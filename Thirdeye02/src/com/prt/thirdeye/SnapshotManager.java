...
public class SnapshotManager {
	...
	public interface SnapshotListener {
		 ...
		 * 
		 * @param info
		 *            A structure containing information about the snapshot
		 *            taken
		 */
		...
		/**
		 * This callback is called when a snapshot is being processed (auto
		...
		/**
		 * This callback is called when a snapshot has been saved to the
		 * internal memory
		 * 
		 * @param info
		 *            A structure containing information about the snapshot
		...
		/**
		 * This callback is called when ImageSaver starts a job of saving an
		 * image, or MediaRecorder is storing a video The primary purpose of
		 * this method is to show the SavePinger
		 */
		public void onMediaSavingStart();

		/**
		 * This callback is called when ImageSaver has done its job of saving an
		 * image, or MediaRecorder is done storing a video The primary purpose
		 * of this method is to hide the SavePinger
		 */
	...
	public class SnapshotInfo {
		...
		// The URI of the saved shot, if it has been saved, and if the save is
		// done (mSave = true).
		// This field will likely be only different than null when
		// onSnapshotSaved is called.
		public Uri mUri;
	...
	private CamManager mCameraManager;
	...
	private Camera.ShutterCallback mShutterCallback = new Camera.ShutterCallback() {
		...
		public void onShutter() {
			...
			final SnapshotInfo snap = mSnapshotsQueue
					.get(mCurrentShutterQueueIndex);
			...
			// TODO: PRAT checked only when it's enabled.
	...
	private Camera.PictureCallback mJpegPictureCallback = new Camera.PictureCallback() {
		...
		public void onPictureTaken(byte[] jpegData, Camera camera) {
			...
			final Camera.Size s = mCameraManager.getParameters()
					.getPictureSize();
			int orientation = mCameraManager.getOrientation();
			final int width = s.width, height = s.height;

			if (mSnapshotsQueue.size() == 0) {
				Log.e(TAG,
						"DERP! Why is snapshotqueue empty? Two JPEG callbacks!?");
				return;
			}
			...
			if (mContext.getResources().getBoolean(R.bool.config_useSamsungHDR)) {
				...
				// TODO: Replace 90 with real JPEG compression level when we'll
				// have that setting
			...
			if (snap.mSave) {
				...
				// If the orientation is somehow negative, avoid the Gallery
				// crashing dumbly
				// (see com/android/gallery3d/ui/PhotoView.java line 758
				// (setTileViewPosition))
				while (orientation < 0) {
					orientation += 360;
				}
	...
	private Runnable mCaptureRunnable = new Runnable() {
		...
		public void run() {
			...
			mCameraManager.takeSnapshot(mShutterCallback, null,
	...
	public SnapshotManager(CamManager man, Context ctx) {
		mContext = ctx;
		mCameraManager = man;
	...
	/**
	 * Sets whether or not to bypass image processing (for burst shot or hdr for
	 * instance) This value is reset after each snapshot queued!
	 * 
	 * @param bypass
	 */
	 ...
	 * 
	 * @param save
	 *            Whether or not to save the snapshot in gallery (for example,
	 *            software HDR doesn't need all the shots to be saved)
	 * @param exposureCompensation
	 *            If the shot has to be taken at a different exposure value,
	 *            otherwise set it to 0
	 */
	public void queueSnapshot(boolean save, int exposureCompensation) {
		if (CamActivity.getCameraMode() == CamActivity.CAMERA_MODE_VIDEO
				&& mContext.getResources().getBoolean(
						R.bool.config_usePreviewForVideoSnapshot)) {
			// We use the preview data for the video snapshot, instead of
			// queueing a normal snapshot
			new Thread(mPreviewCaptureRunnable).start();
			return;
		}

		if (mSnapshotsQueue.size() == 2)
			return; // No more than 2 shots at a time

		// If we use Samsung HDR, we must set exposure level, as it corresponds
		// to the HDR bracket
		// TODO: PRAT
		if (mContext.getResources().getBoolean(R.bool.config_useSamsungHDR)) {
			exposureCompensation = mCameraManager.getParameters()
					.getMaxExposureCompensation();
			mResetExposure = mCameraManager.getParameters()
	...
	public void setVideoProfile(final CamcorderProfile profile) {
		...
		if (CamActivity.getCameraMode() == CamActivity.CAMERA_MODE_VIDEO) {
			...
			mCameraManager.setVideoSize(profile.videoFrameWidth,
	...
	private boolean addVideoToMediaStore() {
		...
		if (mVideoFileDescriptor == null) {
			...
			try {
				...
				String finalName = mCurrentVideoValues
						.getAsString(MediaStore.Video.Media.DATA);
				if (new File(mCurrentVideoFilename)
...
//		}
	...
	private class ImageSaver extends Thread {
		...
		private void storeImage(final byte[] data, Uri uri, String title,
				Location loc, int width, int height, int orientation,
				List<Tag> exifTags, SnapshotInfo snap) {
			boolean ok = Storage.getStorage().updateImage(mContentResolver,
					uri, title, loc, orientation, data, width, height);

			if (ok) {
				if (exifTags != null && exifTags.size() > 0) {
					// Write exif tags to final picture
					try {
						ExifInterface exifIf = new ExifInterface(
								Util.getRealPathFromURI(mContext, uri));

						for (Tag tag : exifTags) {
							// move along
							String[] hack = tag.toString().split("\\]");
							hack = hack[1].split("-");
							exifIf.setAttribute(tag.getTagName(),
									hack[1].trim());
						}
	...
	private static class ImageNamer extends Thread {
		...
		private void generateUri() {
			...
			mUri = Storage.getStorage().newImage(mResolver, mTitle, mDateTaken,
					mWidth, mHeight);
		}
